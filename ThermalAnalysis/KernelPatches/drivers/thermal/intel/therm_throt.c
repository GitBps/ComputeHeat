...
static void get_therm_status_phD(int level, bool *proc_hot, u8 *temp, unsigned long *currFreq)
{
	int msr;
	u64 msr_val;
...
static void __maybe_unused throttle_active_work(struct work_struct *work)
{
	...
	get_therm_status_phD(state->level, &hot, &temp, &currFreq);
	pr_debug("CPU%d: ThermalAnalyzer:throttle_active_work:proc_hot:%d, value:%d Curr_Freq=%ld\n", this_cpu, hot, temp, currFreq);
...
static void therm_throt_process(bool new_event, int event, int level)
{
	...
	if (new_event && !state->last_interrupt_time) {
		...
		get_therm_status_phD(state->level, &hot, &temp, &currFreq);
	        ...
	        //trace_printk("ThermalAnalyzer:CPU%d: ThermalInterrupt, ProcHot=%d, ThermalHR=%d, Curr_Freq=%ld\n",this_cpu, hot, temp,currFreq);
		...
		//if (temp >20)
		//{
		//	trace_printk("ThermalAnalyzer: CPU:CPU%d, Task:x, TaskLW:y, TaskPrevLoadW:z, TaskvRunTime:k, TaskLoadAvgContrib:l", 
		//		       this_cpu);
		//}

		if (temp > 10)
			return;
		//ThermalAnalyzer. baseline temp is nothing but the thermal headroom
		//with reference to the temperature sensor readings ! 
...
// ThermalAnalyzer, this is called from the LVT  - localized vector table in the low level kernel 
